...
pub fn setup_boot_config(mount_dir: &str, uuid_root: &str, cmdline_console: &str, cmdline_extra: &str, config_params: &Vec<&str>) {
    ...
    writeln!(cmdline_txt, "root=UUID={}", uuid_root).expect("Failed to write to cmdline.txt");
    writeln!(cmdline_txt, "{}", cmdline_console).expect("Failed to write to cmdline.txt");
    writeln!(cmdline_txt, "{}", cmdline_extra).expect("Failed to write to cmdline.txt");
